...
const ModelSection = () => {
  ...
  return (
    <section ref={sectionRef} className="min-h-screen w-full bg-black overflow-hidden flex flex-col md:flex-row items-stretch justify-start gap-0">
        <div ref={leftSideRef} className="bg-[#39FF14] flex-col md:flex-[0_0_40%] w-full md:h-screen h-[30vh] flex items-center justify-center opacity-0">
          <img src="/assets/logo.svg" alt="logo" className="h-[15vh] md:h-[20vh] w-auto"/>
        </div>
        <div ref={rightSideRef} className="flex-col md:flex-[0_0_60%] w-full flex flex-col items-start px-6 md:px-10 py-12 md:py-20 justify-center opacity-0 bg-black">
            <h1 ref={h1Ref} className="font-overlord text-[#39FF14] text-[32px] md:text-[45px] text-left scale-95 opacity-0">What is Trendifi</h1>
            <p ref={pRef} className="font-regular text-xs uppercase text-white tracking-widest mt-5 leading-6 opacity-0 max-w-2xl">
            ...
            <button ref={buttonRef} className="bg-[#39FF14] text-black font-overlord text-xs md:text-sm uppercase tracking-widest py-3 px-6 border-2 border-[#39FF14] hover:bg-transparent hover:text-[#39FF14] transition-all duration-300 mt-5 opacity-0 translate-y-4">Get Started</button>
        </div>
    </section>
  )
}
